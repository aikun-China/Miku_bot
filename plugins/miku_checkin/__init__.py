...
from utils.user_store import do_checkin, has_buff, set_buff, BUFF_NEXT_DOUBLE_FAVOR
...
def _use_double_favor_card_effect(bot, event, user_id, name, qty, remaining, key):
    if has_buff(user_id, BUFF_NEXT_DOUBLE_FAVOR):
        return {
            "consume": False,
            "text": (
                "❌ 无法使用双倍好感卡：\n"
                "你已经激活了下次签到双倍好感，无法重复使用。"
            ),
        }

    set_buff(user_id, BUFF_NEXT_DOUBLE_FAVOR)
    return {
        "text": (
            "使用双倍好感卡成功！\n"
            f"你现在还有 {remaining} 张双倍好感卡。\n"
            "下次签到时，系统会自动触发 100% 双倍好感。"
        )
    }

SHOP_ITEM_USE_HANDLERS = {
    "双倍好感卡": _use_double_favor_card_effect,
}
...
def _is_enabled() -> bool:
    raw = config_manager.get("miku_checkin", "enabled", True)
...
async def _checkin_handle(bot, event):
    ...
    # 构建头像 HTML（简单字符串替换不支持 if-else）
    if avatar_uri:
        avatar_html = f'<img class="avatar" src="{avatar_uri}" alt="头像">'
    else:
        avatar_html = f'<div class="avatar-fallback">{(nickname or "U")[0]}</div>'
...
register_plugin_info(
    ...
    icon="📅",
    order=4,
    description="每日签到获得金币和好感度，有概率触发双倍好感",
    commands=["签到", "每日签到", "打卡", "每日打卡"],
    usage="""发送以下指令进行签到：
签到 / 每日签到 / 打卡 / 每日打卡
